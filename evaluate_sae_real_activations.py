...
def load_dataset_samples(dataset_name: str, column_name: str, num_samples: int, 
    ...
    dataset_configs = [
        ...
        ("wikitext", "wikitext-2-raw-v1", False),  # Fallback 1
        ("wikitext", "wikitext-103-raw-v1", False),  # Fallback 2
        ("bookcorpus", None, False),  # Fallback 3
    ]
...
def encode_sae(activations: torch.Tensor, weights: Dict) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    W_enc = weights["W_enc"]  # Shape: (d_in, d_sae) for SAELens
    ...
    pre_acts = F.linear(activations, W_enc.T, b_enc)  # W_enc.T to get (d_sae, d_in)
